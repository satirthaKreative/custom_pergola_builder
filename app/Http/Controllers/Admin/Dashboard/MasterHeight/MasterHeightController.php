...
namespace App\Http\Controllers\Admin\Dashboard\MasterHeight;
...
class MasterHeightController extends Controller
{
    ...
    {
        return view('backend.pages.master-height.master-height');
    }

    public function showPageData(Request $request)
    {
        $showQuery = MasterHeightModel::get();
        if(count($showQuery) > 0)
        {
            ...
            foreach($showQuery as $sqOne)
            {
                ...
                $html .= '<tr>
                            <td>'.++$i.'</td>
                            <td>'.$sqOne->master_height_length.' ft.</td>
                            <td>'.$status.'</td>
    ...
    {
        $checkHeight = MasterHeightModel::where('master_height_length',$request->input('master_height'))->get();
        ...
        {
            $insertArr = [
                'master_height_length' => $request->input('master_height'), 
                'admin_action' => 'yes', 
                'created_at' => date('Y-m-d H:i:s'),
                'updated_at' => date('Y-m-d H:i:s')
                
            ];
            $insertQuery = MasterHeightModel::insert($insertArr);
            if($insertQuery)
            {
    ...
    public function changeActionData(Request $request)
    {
        ...
        $changeQuery = MasterHeightModel::where('id',$_GET['id'])->update(['admin_action' => $state]);
        if($changeQuery)
        {
            $msg = "success";
        }
    ...
    {
        $changeQuery = MasterHeightModel::where('id',$_GET['id'])->delete();
        if($changeQuery)
        {
            $msg = "success";
        }
    ...
    {
        $changeQuery = MasterHeightModel::where('id',$_GET['id'])->get();
        foreach($changeQuery as $cData)
        {
            $mainData = $cData->master_height_length;
        }
    ...
    public function changeActionEdit(Request $request, $my_data)
    {
        ...
        $insertQuery = MasterHeightModel::where('id',$my_data)->update($insertArr);
        if($insertQuery)
        {
