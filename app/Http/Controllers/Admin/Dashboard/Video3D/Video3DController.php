...
class Video3DController extends Controller
{
    ...
    public function editViewChangeActionFx(Request $request)
    {
        ...
        if(count($masterWoodQuery) > 0)
        {
            foreach($masterWoodQuery as $mWquery)
            {
                ...
                foreach($newQ as $nQ)
                {
                    $p_id = $nQ->wood_type_id;
                    if($mWquery->id == $p_id)
                    {
                        $selected = "selected";
                    }
                    
                    $html['master_wood_query'] .= '<option value="'.$mWquery->id.'" '.$selected.'>'.$mWquery->wood_name.'</option>';
                }
            }
        }
    ...
    public function getDataPageFx(Request $request)
    {
        ...
        // wood types
        $masterWoodQuery = MasterWoodModel::where('admin_action','yes')->get();
        $html['master_wood_types_name'] = "<option value=''>Choose Wood Types</option>";
        if(count($masterWoodQuery) > 0)
        {
            foreach($masterWoodQuery as $masterW)
            {
                $html['master_wood_types_name'] .= '<option value="'.$masterW->id.'">'.$masterW->wood_name.'</option>';
            }
        }
    ...
    public function getTableDataPageFx(Request $request)
    {
        ...
        if(count($mainQuery) > 0)
        {
            ...
            foreach($mainQuery as $mQuery)
            {
                ...
                $masterWoodQuery = MasterWoodModel::where(['id' => $mQuery->wood_type_id ])->get();
                if(count($masterWoodQuery) > 0)
                {
                    foreach($masterWoodQuery as $masterWQuery)
                    {
                        $wood_types = $masterWQuery->wood_name;
                    }
                }
                else
                {
                    $wood_types = "No wood type specified";
                }
        ...
        {
            $html['show_3d_show'] = '<tr><td colspan="9"><center class="text-danger"><i class="fa fa-trash"></i> No data yet</center></td></tr>';
        }
    ...
    {
        $checkQuery = Video3DModel::where(['master_width' => $request->input('master_width') , 'master_height' => $request->input('master_length'), 'master_posts' => $request->input('no_of_posts') , 'master_overhead' => $request->input('overhead_shades'), 'wood_type_id' => $request->input('wood_types_name') ])->get();
    ...
    {
        $checkQuery = Video3DModel::where('id','!=',$my_data)->where(['master_width' => $request->input('master_width') , 'master_height' => $request->input('master_length'), 'master_posts' => $request->input('no_of_posts') , 'master_overhead' => $request->input('overhead_shades'), 'wood_type_id' => $request->input('wood_types_name') ])->get();
        if(count($checkQuery) > 0)
        {
