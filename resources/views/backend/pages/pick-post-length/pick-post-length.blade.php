          ...
          <div class="card card-primary">
            ...
            @csrf
              ...
              <div class="form-group">
                ...
                <div class="form-group">
                  <label for="overhead-shades-id">Wood Types</label>
                  <select required class="form-control" name="wood_types_name" id="wood-types-name-id" onclick="master_width_onchange()">
                    <option value="">Choose wood types</option>
                  </select>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <select required class="form-control" name="post_length" id="master-post-length-id" placeholder="Enter post length" onchange="post_length_changes_pricing_onchange_fx()">
                ...
                <!-- <div class="form-group">
                  ...
                  <input type="hidden" class="form-control" name="post_length_price" id="ladder-spacing-price-id" placeholder="Enter post length price" />
                <!-- </div> -->
                <div class="form-group">
...
<script>
    ...
    // config 
    function post_length_changes_pricing_onchange_fx()
    {
      var master_width = $("#master-width-id").val();
      var master_height = $("#master-height-id").val();
      var master_post = $("#master-post-id").val();
      var master_overhead = $("#ladder-spacing-id").val();
      var master_post_length = $("#master-post-length-id").val();

      if(master_width != "" && master_height != "" && master_post != "" && master_post_length != "" )
      {
        $.ajax({
          url: "{{ route('admin.show-pick-a-post-length-changes-config') }}",
          type: "GET",
          data: {post_id: master_post, width_val: master_width, height_val: master_height, master_post_length: master_post_length},
          dataType: "json",
          success: function(event)
          {
            $("#ladder-spacing-price-id").val(event.last_main_price);
          }, 
    ...
    function master_width_onchange()
    {
      ...
      var wood_id = $("#wood-types-name-id").val();

      if(w_id != "" && w_id != null && h_id != "" && h_id != null && wood_id != "" && wood_id != null && (p_id == null || p_id == ""))
      {
        $.ajax({
          url: "{{ route('admin.pick-overhead-postLength-shades-post-first-load') }}",
          type: "GET",
          data: {w_id: w_id, h_id: h_id, wood_id: wood_id},
          dataType: 'json',
          success: function(event){
            $("#master-post-id").html(event.master_posts);
            post_length_changes_pricing_onchange_fx();
          }, error: function(event){

          }
        });
      }
      else if(w_id != "" && w_id != null && h_id != "" && h_id != null && p_id != null && p_id != "" && wood_id != "" && wood_id != null)
      {
        $.ajax({
          url: "{{ route('admin.pick-overhead-postLength-shades-post-first-load') }}",
          type: "GET",
          data: {w_id: w_id, h_id: h_id, p_id: p_id, wood_id: wood_id},
          dataType: 'json',
          success: function(event){
    ...
    function load_master_wood_types_fx()
    {
      $.ajax({
        url: "{{ route('admin.pick-post-length-wood-type') }}",
        type: "GET",
        dataType: "json",
        success: function(event){
          $("#wood-types-name-id").html(event.wood_types_name);
        }, error: function(event){
    ...
    function make_edit_change(id)
    {
        ...
        $.ajax({
            ...
            success: function(event){
                ...
                $("#master-height-id").html(event.master_height);
                $("#master-height-id").attr('onchange','load_config_master_height_fx()');
                $("#master-width-id").html(event.master_width);
                $("#master-width-id").attr('onchange','load_config_master_width_fx()');
                $("#ladder-spacing-id").html(event.master_overhead_shades);
                $("#master-post-id").html(event.master_posts);
                $("#master-post-id").attr('onchange','load_config_master_post_fx()');
                $("#master-post-length-id").html(event.master_post_length_data);
                $("#wood-types-name-id").html(event.master_wood_query);
                ...
                load_edit_pick_post_length_onchange_fx();
            }, error: function(event){
    ...
    function load_config_master_post_fx()
    {
      var master_width = $("#master-width-id").val();
      var master_height = $("#master-height-id").val();
      var master_post = $("#master-post-id").val();
    ...
    function load_config_master_width_fx()
    {
      var master_width = $("#master-width-id").val();
      var master_height = $("#master-height-id").val();
      var master_post = $("#master-post-id").val();
    ...
    function load_config_master_height_fx()
    {
      var master_width = $("#master-width-id").val();
      var master_height = $("#master-height-id").val();
      var master_post = $("#master-post-id").val();
    ...
    function load_edit_pick_post_length_onchange_fx()
    {
      var master_width = $("#master-width-id").val();
      var master_height = $("#master-height-id").val();
      var master_post = $("#master-post-id").val();
