      ...
      <div class="row mb-2">
        ...
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">Master Width</li>
          </ol>
        </div>
      </div>
    </div><!-- /.container-fluid -->
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="card-header">
              <h3 class="card-title">Master Width Table</h3>
        ...
        <div class="col-md-6" id="add-master-height">
          ...
          <div class="card card-primary">
            <div class="card-header">
              <h3 class="card-title" id="add-edit-master-height-id">Master Width ( Add Form ) </h3>
            </div>
            <!-- /.card-header -->
            <!-- form start -->
            <form role="form" enctype="multipart/form-data" id="add-edit-master-height-action-id" method="POST" action="{{ route('admin.master-width-submit') }}">
            @csrf
              <div class="card-body">
                <div class="form-group">
                  <label for="ladder-spacing-id">Width *</label>
                  <input type="text" class="form-control" name="master_height" id="master-height-id" placeholder="Enter master width" />
                </div>
              </div>
              <!-- /.card-body -->
...
<script>
    ...
    {
        $.ajax({
            url: "{{ route('admin.master-width-show') }}",
            type: "GET",
            dataType: "json",
    ...
    {
        $.ajax({
            url: "{{ route('admin.master-width-action-change') }}",
            type: "GET",
    ...
    {
        $.ajax({
            url: "{{ route('admin.master-width-action-del') }}",
            type: "GET",
            data: {id: id},
            dataType: "json",
            success: function(event){
    ...
    {
        var url_id= "{{ url('admin/master-width-action-edit') }}"+"/"+id;
        $.ajax({
            url: "{{ route('admin.master-width-action-get-edit') }}",
            type: "GET",
            data: {id: id},
            dataType: "json",
            success: function(event){
                $("#add-edit-master-height-id").html("Master Width ( Edit Form )");
