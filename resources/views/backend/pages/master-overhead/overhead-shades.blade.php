...
<section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>Master Overhead Shades</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Home</a></li>
            <li class="breadcrumb-item active">Master Overhead Shades</li>
          </ol>
        </div>
      </div>
    </div><!-- /.container-fluid -->
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="card-header">
              <h3 class="card-title">Master Overhead Table</h3>
              ...
              <span class="float-right"><a class="btn btn-danger btn-sm" href="#add-master-height" id="add-master-height-id" style="margin-right: 10px;">Add Overhead Shades</a></span>
            </div>
        ...
        <div class="col-md-6" id="add-master-height">
          ...
          <div class="card card-primary">
            <div class="card-header">
              <h3 class="card-title" id="add-edit-master-height-id">Master Overhead Shades ( Add Form ) </h3>
            </div>
            <!-- /.card-header -->
            <!-- form start -->
            <form role="form" enctype="multipart/form-data" id="add-edit-master-height-action-id" method="POST" action="{{ route('admin.master-overhead-shades-show-actual-submit') }}">
            @csrf
              <div class="card-body">
                <div class="form-group">
                  <label for="ladder-spacing-id">Overhead Shades *</label>
                  <input type="text" class="form-control" name="master_height" id="master-height-id" placeholder="Enter Overhead Shades" />
                </div>
              </div>
              <!-- /.card-body -->
...
<script>
    $(function(){
        ...
        $("#add-master-height-id").click(function(){
          $("#add-edit-master-height-id").html("Overhead Shades ( Add Form )");
          $("#add-edit-master-height-action-id").find('#master-height-id').val("");
          $("#add-edit-master-height-action-id").attr("action","{{ route('admin.master-overhead-shades-show-actual-submit') }}");
        })
    });
    ...
    {
        $.ajax({
            url: "{{ route('admin.master-overhead-shades-show-actual-data') }}",
            type: "GET",
            dataType: "json",
    ...
    {
        $.ajax({
            url: "{{ route('admin.master-overhead-shades-show-action-change-data') }}",
            type: "GET",
    ...
    {
      var x = confirm('Are you really want to delete this item?');
      if(x){
        $.ajax({
            url: "{{ route('admin.master-overhead-shades-action-del') }}",
            type: "GET",
            data: {id: id},
            dataType: "json",
            success: function(event){
    ...
    {
        var url_id= "{{ url('admin/master-overhead-shades-action-edit') }}"+"/"+id;
        $.ajax({
            url: "{{ route('admin.master-overhead-shades-action-get-edit') }}",
            type: "GET",
            data: {id: id},
            dataType: "json",
            success: function(event){
                $("#add-edit-master-height-id").html("Overhead Shades ( Edit Form )");
