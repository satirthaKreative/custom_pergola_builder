...
<script>
    ...
    function posts_in_feet_master_fx()
    {
      ...
      else
      {
        ...
        {
          $("#pick-up-footprint-img-body-id").append('<div class="form-group"><label for="img-in-feet-master-id">Image (Optional)</label><input type="file" class="form-control" name="img_in_feet_master" id="img-in-feet-master-id" placeholder="Choose a image"></div><div class="form-group"><label for="price-in-feet-master-id" style="display:none">Price *</label><input type="hidden"  class="form-control" name="price_in_feet_master" id="price-in-feet-master-id" placeholder="Enter Product Price" required></div><div class="form-group"><label for="master-pickup-footprint-wood-type-id">Wood Type Choose *</label><select class="form-control" name="master_pickup_wood_name" id="master-pickup-footprint-wood-type-id" placeholder="Enter Product Wood Name" required></select></div>');
          $("#pickup-submit-btn").prop('disabled', false);
          load_price_using_math_formula_fx(get_posts_id);
          loading_pickup_wood_type_choose_fx();
          loading_price_generating_value_choose_fx();
        }
    ...
    function loading_price_generating_value_choose_fx()
    {
      var width_val = $("#width-in-feet-master").val();
      var height_val = $("#height-in-feet-master").val();
      var posts_val = $("#posts-in-feet-master-id").val();
      $.ajax({
        url: "{{ route('admin.loading-add-price-generating-value-choose') }}",
        type: "GET",
        data: { width_val: width_val, height_val: height_val, posts_val: posts_val },
        dataType: "json",
        success: function(event){
          $("#price-in-feet-master-id").val(event.add_main_price);
        }, error: function(event){
    ...
    function loading_pickup_wood_type_choose_fx()
    {
      $.ajax({
        url: "{{ route('satirtha.pick-a-footprint-choose-wood-type') }}",
        type: "GET",
        dataType: "json",
        success: function(event){
          $("#master-pickup-footprint-wood-type-id").html(event.wood_types);
        }, error: function(event){
    ...
    function load_price_using_math_formula_fx(post_id)
    {
      var height_val = $("#height-in-feet-master").val();
      var width_val = $("#width-in-feet-master").val();
      $.ajax({
        ...
        type: "GET",
        data: {post_id: post_id, width_val: width_val, height_val: height_val},
        dataType: "json",
        success: function(event)
        {
