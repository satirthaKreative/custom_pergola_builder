              ...
              <div class="card-body card_panel_body">
                <div class="form-group">
                  <label for="ladder-spacing-id">Combination Panel *</label>
                    <select class="form-control" name="combination_name" id="combination-panel-id" onchange="combination_config_onchange_fx()" required>
                            <option value="">Choose combination panel</option>
                    </select>
                </div>
                <!-- <div class="form-group">
                  <h4 for="ladder-spacing-id">POST MOUNT BRACKET *</h4>
                  <label for="ladder-spacing-id">Mount Price *</label> -->
                    <input type="hidden" class="form-control" name="exist_price" id="mount-price-id" required>
                ...
                <!-- <div class="form-group">
                  ...
                  <label for="ladder-spacing-id">Canpony Price *</label> -->
                    <input type="hidden" class="form-control" name="canopy_price" id="canopy-price-id" required>
                <!-- </div>
                <div class="form-group">
                  <h4 for="ladder-spacing-id">POST LOUVERED PANEL *</h4>
                  <label for="ladder-spacing-id">Louvered price*</label> -->
                    <input type="hidden" class="form-control" required name="lpanel_price" id="lpanel-price-id">
...
<script>
    ...
    /// config config panel
    function combination_config_onchange_fx()
    {
      var config_id = $("#combination-panel-id").val();
     
      if(config_id == "" || config_id == null)
      {
        $("#mount-price-id").val(""); 
        $("#canopy-price-id").val(""); 
        $("#lpanel-price-id").val("");
      ...
      {
        $.ajax({
          url: "{{ route('admin.new-config-combination-panel') }}",
          type: "GET",
          data: {combination_id: config_id},
          dataType: "json",
          success: function(event){
            $("#mount-price-id").val(event.mount_bracket_price);
            $("#canopy-price-id").val(event.canopy_price);
            $("#lpanel-price-id").val(event.louvered_panel_price);
    ...
    function make_edit_change(id)
    {
        ...
        $.ajax({
            ...
            success: function(event){
                ...
                $("#mount-price-id").val(event.exist_price);

                $("#canopy-price-id").val(event.canopy_price);

                $("#lpanel-price-id").val(event.left_price);

                load_edit_config_show_combination_fx();
            }, error: function(event){
    ...
    function load_edit_config_show_combination_fx()
    {
      var config_id = $("#combination-panel-id").val();
