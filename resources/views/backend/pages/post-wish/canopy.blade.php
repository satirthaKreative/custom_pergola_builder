    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        @csrf
            ...
            <div class="form-group">
                <label for="image-links-id">Image File: <b class="text-danger">( file size not more than 2mb * )</b></label>
                <input type="file"  name="canopy_image_file_name" class="form-control" placeholder="Enter the image links" id="image-links-id" >
            </div>
            <div class="form-group">
        ...
        @csrf
          <input type="hidden" name="canopy_name_hidden_id" id="canopy-name-hidden-id" value="" /> 
            ...
            <div class="form-group">
                ...
                <div class="canopy-videos-listing-class">

                </div>
            </div>
            <div class="form-group">
                <label for="video-links-id">Image File: <b class="text-danger">( file size not more than 2mb * )</b></label>
                <input type="file"  name="edit_canopy_image_file_name" class="form-control" placeholder="Enter the image links" id="image-links-id" >
                <div class="canopy-images-listing-class">
...
<div class="modal" id="pickup-popup-mount-modal-edit-id">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <!-- Modal Header -->
      <div class="modal-header">
        <h4 class="modal-title">Pickup Popup Canopy Edit</h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>

      <!-- Modal body -->
      <div class="modal-body">
        <form role="form" action="{{ route('admin.setup.popup.edit-post-canopy-submit') }}" enctype='multipart/form-data' method="POST">
        @csrf
...
<div class="modal" id="pickup-popup-mount-modal-add-id">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <!-- Modal Header -->
      <div class="modal-header">
        <h4 class="modal-title">Pickup Popup Canopy Add</h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>

      <!-- Modal body -->
      <div class="modal-body">
        <form role="form" action="{{ route('admin.setup.popup.add-post-canopy-submit') }}" enctype='multipart/form-data' method="POST">
        @csrf
        <input type="hidden" name="popup_hidden_id" class="popup-hidden-id" />
            <div class="form-group">
                <label for="image-links-id">Popup Name:</label>
...
<script>
    $(function(){
        ...
        CKEDITOR.replace( 'canopy_description_name' );
    ...
    function canopy_edit_fx(id)
    {
      ...
      $.ajax({
        ...
        success: function(e)
        {
            ...
            {
              $(".canopy-images-listing-class").html('<img src="'+e.img_files+'" alt="no image" width="100px" height="100px" /><a href="javascript: ;" style="color: red; float: left; position: absolute; left: 20px; " onclick=onImgDelete('+e.imgId+')><i class="fa fa-trash"></i></a>');
            }
            else
            {
              $(".canopy-images-listing-class").html('');
            }

            if(e.video_files_id != null)
            {
              $(".canopy-videos-listing-class").html('<video width="100" height="100" controls><source src="'+e.video_files+'" type="video/mp4"></video><a href="javascript: ;" style="color: red; float: left; position: absolute; left: 20px; " onclick=onVideoDelete('+e.imgId+')><i class="fa fa-trash"></i></a>');
            }
            else
            {
              $(".canopy-videos-listing-class").html('');
            }
        }, error: function(e)
        {
    ...
    {
      $.ajax({
        url: "{{ route('admin.canopy-img-for-post-del-fx') }}",
        type: "GET",
        data: {id: id},
        dataType: "json",
        success: function(event)
    ...
    {
      $.ajax({
        url: "{{ route('admin.canopy-video-for-post-del-fx') }}",
        type: "GET",
        data: {id: id},
        dataType: "json",
        success: function(event)
    ...
    {
      $.ajax({
        url: "{{ route('admin.popup.post-canopy-window-edit-data') }}",
        type: "GET",
        data: {id: id},
        dataType: "json",
        success: function(event){
